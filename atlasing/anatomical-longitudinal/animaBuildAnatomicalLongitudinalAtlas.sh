...
do
    ...
    do
        rm -f residualDir/${prefix}_${a}_{compo_tr.nii.gz,flag}
    done
    ...
    echo "#OAR -p dedicated='none' or dedicated = 'neurinfo'">> tmpRun_${k}
    ...
    if [ ${k} -eq 1 ]; then
        ...
        echo "${ROOT_DIR}/atlasing/anatomical-longitudinal/animaLongitudinalRegisterImage.sh ${PWD} ${ref}.nii.gz ${prefixBase} ${prefix} \$index ${bchOrder} ${ncores}" >> tmpRun_${k}
    else
        echo "${ROOT_DIR}/atlasing/anatomical-longitudinal/animaLongitudinalRegisterImage.sh ${PWD} ${ref}.nii.gz ${prefixBase} ${prefix} \$OAR_ARRAY_INDEX ${bchOrder} ${ncores}" >> tmpRun_${k}
    fi
    ...
    echo "#OAR -l /nodes=1/core=20,walltime=01:59:00" >> mergeRun_${k}
    echo "#OAR -p dedicated='none' or dedicated = 'neurinfo'">> mergeRun_${k}
    ...
    echo "${ROOT_DIR}/atlasing/anatomical-longitudinal/animaLongitudinalMergeImages.sh ${PWD} ${prefixBase} ${prefix} ${k} ${nimages} ${ref} ${ncores} ${weights}" >> mergeRun_${k}
