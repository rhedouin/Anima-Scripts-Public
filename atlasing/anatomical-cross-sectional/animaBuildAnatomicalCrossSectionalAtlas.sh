...
do
  if [ -e it_${k}_done ]; then
	...
	if [ ${k} -eq 1 ]; then
		...
		echo "${ROOT_PUBLIC_DIR}/atlasing/anatomical-cross-sectional/animaCrossSectionalRegisterImage.sh ${PWD} ${ref}.nii.gz ${prefixBase} ${prefix} \$index ${ncores}" >> tmpRun_${k}
	else
		echo "${ROOT_PUBLIC_DIR}/atlasing/anatomical-cross-sectional/animaCrossSectionalRegisterImage.sh ${PWD} ${ref}.nii.gz ${prefixBase} ${prefix} \$OAR_ARRAY_INDEX ${ncores}" >> tmpRun_${k}
	fi
	...
	echo "${ROOT_PUBLIC_DIR}/atlasing/anatomical-cross-sectional/animaCrossSectionalMergeImages.sh ${PWD} ${prefixBase} ${prefix} ${k} ${nimages} ${ref} ${ncores}" >> mergeRun_${k}
