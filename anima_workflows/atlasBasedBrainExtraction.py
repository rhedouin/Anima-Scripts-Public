...
import anatomicalNonLinearRegistration as wf_creator


def create_atlas_based_brain_extraction_workflow(name='anima_brain_extraction',
                                                 temp_registered_atlas_img_file='atlas_reg.nrrd'):
    wf = Workflow(name=name)
    wf.base_output_dir = name

    input_node = Node(niu.IdentityInterface(fields=['input_file', 'atlas_img_file', 'atlas_icc_file',
                                                    'registered_atlas_img_file', 'out_masked_file', 'out_mask_file']),
                      name='input_node')

    input_node.inputs.registered_atlas_img_file = temp_registered_atlas_img_file
    ...
    wf.connect(input_node, 'registered_atlas_img_file', anat_reg, 'input_node.out_file')
